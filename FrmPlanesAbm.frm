...
Else
   ...
   sql = "UPDATE planes SET nombre='" & CStr(TxtNombre.Text) & "',tasa1=" & ConvertirDblSql(CDbl(TxtTasa1.Text)) & _
         ...
         ",CantDiasVenc=" & CLng(txtCantDias.Text) & _
...
sql = "SELECT planes.IDplan,planes.nombre AS 'plan'," & _
      ...
      "planes.aplicarseguro,planes.aplicarseguroscuota1,planes.noaplicarsegurosrefinanciacion,planes.importeseguro,planes.alicuotaseguros,planes.importesegurosfijos,planes.porcprodentre, planes.cantdiasvenc" & _
      " FROM planes " & _
      "WHERE Idplan=" & CLng(lv.SelectedItem)
...
If Not rec.EOF Then
  ...
  If rec.rdoColumns("noaplicarotorrefin") Then
   ...
   If rec.rdoColumns("aplicarseguro") Then
    ...
    End If
    If IsNull(rec.rdoColumns("cantdiasvenc")) Then
        txtCantDias = 0
    Else
        txtCantDias = rec.rdoColumns("cantdiasvenc")
    End If
...
Else
    If Not IsNumeric(Trim(txtCantDias.Text)) Then
        datosok = False
        MsgE "La cantidad de dias de fecha limite de Vencimiento debe ser un número"
