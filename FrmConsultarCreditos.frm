...
Begin VB.Form FrmConsultarCreditos 
   ...
   ClientWidth     =   10800
   HelpContextID   =   17
   Icon            =   "FrmConsultarCreditos.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.ComboBox ComboOpciones 
         Height          =   315
         ItemData        =   "FrmConsultarCreditos.frx":628A
         Left            =   120
         List            =   "FrmConsultarCreditos.frx":629D
      ...
      Begin VB.Frame FrameCliente 
         ...
         Begin VB.CommandButton CmdBuscarCliente 
            Height          =   350
            Left            =   5640
            Picture         =   "FrmConsultarCreditos.frx":62F3
            Style           =   1  'Graphical
            TabIndex        =   1
      ...
      Begin VB.Frame FrameConsultaMasiva 
         ...
         Begin VB.CommandButton CmdBuscar 
            Height          =   255
            Left            =   4800
            Picture         =   "FrmConsultarCreditos.frx":66AE
            Style           =   1  'Graphical
            TabIndex        =   4
   ...
   Begin VB.Frame Frame6 
      ...
      Top             =   560
      Width           =   10695
      Begin VB.CommandButton cmdInfoBloqueo 
         Caption         =   "Info. Bloqueo"
         Enabled         =   0   'False
         Height          =   255
         Left            =   9120
         TabIndex        =   105
         ToolTipText     =   "Actualiza los datos del crédito bloqueado"
         Top             =   1860
         Width           =   1455
      End
      Begin VB.ComboBox ComboProvincias 
         ...
         Enabled         =   0   'False
         Height          =   315
         ItemData        =   "FrmConsultarCreditos.frx":67F8
         Left            =   7560
         List            =   "FrmConsultarCreditos.frx":67FA
         Style           =   2  'Dropdown List
         TabIndex        =   79
         ...
         Width           =   1455
      End
      Begin VB.CheckBox CheckComercio 
         Caption         =   "Comercio"
         Enabled         =   0   'False
         Height          =   255
         Left            =   9120
      ...
      Begin VB.ComboBox ComboComercios 
         ...
         Enabled         =   0   'False
         Height          =   315
         ItemData        =   "FrmConsultarCreditos.frx":67FC
         Left            =   9120
         List            =   "FrmConsultarCreditos.frx":67FE
         Style           =   2  'Dropdown List
         TabIndex        =   103
         ToolTipText     =   "Lista de provincias"
         Top             =   2400
   ...
   Begin VB.Frame FrameFiltro 
      ...
      Begin VB.ComboBox ComboOrden 
         Height          =   315
         ItemData        =   "FrmConsultarCreditos.frx":6800
         Left            =   120
         List            =   "FrmConsultarCreditos.frx":680D
         Style           =   2  'Dropdown List
         TabIndex        =   72
      ...
      Begin VB.ComboBox ComboOpcionesCuotas 
         Height          =   315
         ItemData        =   "FrmConsultarCreditos.frx":685C
         Left            =   120
         List            =   "FrmConsultarCreditos.frx":6878
         Style           =   2  'Dropdown List
         TabIndex        =   13
...
Private Sub CheckComercio_Click()
Call RefreshTimer

If CheckComercio.Value = 1 Then
   ComboComercios.Enabled = True
   ComboComercios.BackColor = vbWhite
Else
   ComboComercios.Enabled = False
   ComboComercios.ListIndex = -1
   ComboComercios.BackColor = &HFFFFC0
End If
End Sub



Private Sub cmdInfoBloqueo_Click()
Dim sql As String
...
If Not CreditoBloqueado1(IdCredito) Then
   MsgE "El credito no se encuentra bloqueado"
   Exit Sub
End If

Load frmInfoBloqueado
frmInfoBloqueado.Tag = "InfoBloqueo"
frmInfoBloqueado.CmdGrabar.Caption = "Actualizar Info"
frmInfoBloqueado.lblCliente = lvCreditos.SelectedItem.SubItems(2)
frmInfoBloqueado.lblCapital = "$" + lvCreditos.SelectedItem.SubItems(5)
frmInfoBloqueado.lblCuotas = lvCreditos.SelectedItem.SubItems(4)
frmInfoBloqueado.lblFecha = lvCreditos.SelectedItem.SubItems(3)
frmInfoBloqueado.lblIdCredito = lvCreditos.SelectedItem.SubItems(1)

sql = "select codprestamo from creditos where idcredito = " & lvCreditos.SelectedItem.SubItems(1)
Set rec = cnSQL.OpenResultset(sql)
If Not rec.EOF Then
    frmInfoBloqueado.lblCodPrestamo = rec.rdoColumns("codprestamo")
Else
   MsgE "Error al buscar código de préstamo "
   End
End If

sql = "SELECT * from CREDITOSBLOQUEADOS " & _
      "WHERE IDCREDITO=" & IdCredito & " AND SECUENCIA=(SELECT MAX(SECUENCIA) FROM CREDITOSBLOQUEADOS WHERE IDCREDITO=" & IdCredito & ")"
Set rec = cnSQL.OpenResultset(sql)
If Not rec.EOF Then
    If Not IsNull(rec.rdoColumns("FechaEnvio")) Then
        frmInfoBloqueado.dtpFechaEnvio = Format(rec.rdoColumns("FechaEnvio"), "dd/mm/yyyy")
    End If
    If Not IsNull(rec.rdoColumns("IdEstudio")) Then
        For J = 0 To frmInfoBloqueado.cboEstudio.ListCount - 1
            If CLng(frmInfoBloqueado.cboEstudio.ItemData(J)) = rec.rdoColumns("IdEstudio") Then
                frmInfoBloqueado.cboEstudio.ListIndex = J
                Exit For
            End If
        Next
    Else
        'buscar estudio predeterminado (primero el de la provincia del cliente y si no se encuentra el predeterminado. Finalmente, el primero)
        IdEstudio = 0
        sql = "SELECT ESTUDIOS.IDEstudio from CREDITOS,CLIENTES,PROVINCIAS,LOCALIDADES,ESTUDIOS " & _
              "WHERE IDCREDITO=" & IdCredito & " AND CREDITOS.IDCliente=CLIENTES.IDCliente AND " & _
              "CLIENTES.IdLocalidad = LOCALIDADES.IdLocalidad And LOCALIDADES.IdProvincia = PROVINCIAS.IdProvincia " & _
              "AND PROVINCIAS.IDProvincia=ESTUDIOS.IDProvincia"
        Set rec2 = cnSQL.OpenResultset(sql)
        If Not rec2.EOF Then
            IdEstudio = rec2.rdoColumns("IdEstudio")
        Else
            sql = "SELECT IdEstudio from ESTUDIOS " & _
        ...
        If IdEstudio <> 0 Then
            For J = 0 To frmInfoBloqueado.cboEstudio.ListCount - 1
                If CLng(frmInfoBloqueado.cboEstudio.ItemData(J)) = IdEstudio Then
                    frmInfoBloqueado.cboEstudio.ListIndex = J
                    Exit For
                End If
            Next
        End If
    End If
    frmInfoBloqueado.txtObservaciones = rec.rdoColumns("Observaciones")
    If Not IsNull(rec.rdoColumns("Situacion")) Then
        If Trim$(rec.rdoColumns("Situacion")) = "" Then
            frmInfoBloqueado.cboSituacion.ListIndex = 0
        Else
            frmInfoBloqueado.cboSituacion.Text = Trim$(rec.rdoColumns("Situacion"))
        End If
    Else
        frmInfoBloqueado.cboSituacion.ListIndex = 0
    End If
    Select Case rec.rdoColumns("Estado")
        Case "EE"
            frmInfoBloqueado.cboEstado.Text = "Enviar a Estudio"
        Case "NE"
            frmInfoBloqueado.cboEstado.Text = "No Enviar"
            frmInfoBloqueado.dtpFechaEnvio = Format(Now(), "dd/mm/yyyy")
        Case "PE"
            frmInfoBloqueado.cboEstado.Text = "Pendiente de Envio"
            frmInfoBloqueado.dtpFechaEnvio = Format(Now(), "dd/mm/yyyy")
    End Select
Else
    frmInfoBloqueado.cboEstado.ListIndex = -1
    frmInfoBloqueado.txtObservaciones.Text = ""
    frmInfoBloqueado.dtpFechaEnvio = Format(Now(), "dd/mm/yyyy")
    frmInfoBloqueado.dtpFechaEnvio.Enabled = False
    frmInfoBloqueado.cboEstudio.Enabled = False
    ...
    Set rec = cnSQL.OpenResultset(sql)
    If Not rec.EOF Then
        IdEstudio = rec.rdoColumns("IdEstudio")
    Else
        sql = "SELECT IdEstudio from ESTUDIOS " & _
              "WHERE Predeterminado=1"
        Set rec = cnSQL.OpenResultset(sql)
...
End If
Call CenterForm(frmInfoBloqueado)
frmInfoBloqueado.Show

Exit Sub
merror:
tratarerrores "Error abriendo pantalla de info bloqueo"
End Sub
...
Function GenerarSellosTXT(Provincia As String)
    ...
    If Provincia = "CHACO" Then
        If nMes = 1 Then
            cPeriodoChaco = Format(nAnio - 1, "0000") & "12"
        Else
            cPeriodoChaco = Format(nAnio, "0000") & Format(nMes - 1, "00")
        End If
        cNombreArchivo = "RetSellV" & "30-71029451-4" & "00" & cPeriodoChaco & ".txt"
    End If
    ...
    Open cArchivoCompleto For Output As #1
            ...
            If Provincia = "ENTRE RIOS" Then
                ...
                cHeader = Periodo & ";502;" & "30-71029451-4;05;" & "RRM"
                Print #1, cHeader
                For I = 1 To lvCreditos.ListItems.Count
                    Dia = Format(CStr(Day(lvCreditos.ListItems(I).SubItems(3))), "00")
                    Mes = Format(CStr(Month(lvCreditos.ListItems(I).SubItems(3))), "00")
            ...
            If Provincia = "CHACO" Then
                ...
                For I = 1 To lvCreditos.ListItems.Count
                    ...
                    cDetalle = "30710294514" & Space(20 - Len(lvCreditos.ListItems(I))) & lvCreditos.ListItems(I) & FechaCredito & "ART.15-23 " & "1" & Format(StrDni, "00000000000") & Space(25 - Len(StrCliente)) & StrCliente & Format(impCredito * 100, "0000000000") & "0" & "0000" & "00000000000000000000000000000000000000000000000000" & "00000000000000000000000000000000000000000000000000" & Format(impRetenido * 100, "0000000000") & "000000" & Format(impRetenido * 100, "0000000000")
                    Print #1, cDetalle
                Next I
            End If
...
Call RefreshTimer
If BloquearCredito() Then

    Load frmInfoBloqueado
    frmInfoBloqueado.Tag = "Bloqueo"
    frmInfoBloqueado.CmdGrabar.Caption = "Bloquear Crédito"
    frmInfoBloqueado.lblCliente = lvCreditos.SelectedItem.SubItems(2)
    frmInfoBloqueado.lblCapital = "$" + lvCreditos.SelectedItem.SubItems(5)
    frmInfoBloqueado.lblCuotas = lvCreditos.SelectedItem.SubItems(4)
    frmInfoBloqueado.lblFecha = lvCreditos.SelectedItem.SubItems(3)
    ...
    IdEstudio = 0
    sql = "SELECT ESTUDIOS.IDEstudio from CREDITOS,CLIENTES,PROVINCIAS,LOCALIDADES,ESTUDIOS " & _
          "WHERE IDCREDITO=" & lvCreditos.SelectedItem.SubItems(1) & " AND CREDITOS.IDCliente=CLIENTES.IDCliente AND " & _
          "CLIENTES.IdLocalidad = LOCALIDADES.IdLocalidad And LOCALIDADES.IdProvincia = PROVINCIAS.IdProvincia " & _
          "AND PROVINCIAS.IDProvincia=ESTUDIOS.IDProvincia"
    Set rec = cnSQL.OpenResultset(sql)
    If Not rec.EOF Then
...
Exit Sub
merror:
tratarerrores "Error abriendo pantalla de bloqueo"
End Sub
...
If CheckComercio.Value = 1 Then
   If Trim(ComboComercios.Text) <> "" Then
     Sqlcondicion = Sqlcondicion & " and creditos.motivobloqueo='" & CStr(ComboComercios.Text) & "'"
   End If
End If
  ...
  ' End If
   ...
   Nitem.SubItems(26) = rec.rdoColumns("codigobarras") & vbNullString
    ...
    If Not IsNull(rec.rdoColumns("origen")) Then
        Nitem.SubItems(27) = rec.rdoColumns("origen")
    Else
        If rec.rdoColumns("pagofacil") Then
           Nitem.SubItems(27) = "PMC/ANTICIPO"
        End If
        If rec.rdoColumns("rapipago") Then
           Nitem.SubItems(27) = "PF/RP"
        End If
    End If
    Nitem.SubItems(28) = ""
    If Not IsNull(rec.rdoColumns("idcobrador")) Then
        Nitem.SubItems(28) = NombreCobrador(rec.rdoColumns("idcobrador"))
    End If
...
On Error GoTo merror
    ...
    sql = "select cuotas.* from cuotas, creditos " & _
          ...
          "order by cuotas.fechavencimiento1,cuotas.fechavencimiento2"
    ...
    Do While Not rec.EOF
        ...
        Do While nMesActual = nMesActualLeido And _
            ...
            ImporteParcial = ImporteParcial + ObtenerImporteParcialX(rec.rdoColumns("idcredito"), rec.rdoColumns("numcuota"))
...
sql = "select provincias.nombre as provincia,localidades.nombre as localidad," & _
      ...
      "where creditos.idcredito = " & IdCredito & _
      " order by cuotastemp.numcuota"
...
Dim CondicionOrden As String
Dim CondicionProvincia As String
Dim CondicionComercio As String
Dim IdProvincia As Long
On Error GoTo merror
...
'por defecto todos los comercios
CondicionComercio = " and 1=1"
If CheckComercio.Value = 1 Then
   'si hay un comercio seleccionado
   If Trim$(ComboComercios.Text) <> "" Then
      CondicionComercio = " and creditos.motivobloqueo='" & CStr(ComboComercios.Text) & "'"
   End If
End If
...
sql = "select provincias.nombre as provincia,localidades.nombre as localidad," & _
      ...
      "(cuotas.importevencimiento1) as importetotal,cuotas.importevencimiento2,cuotas.origen,cuotas.idcobrador " & _
      "from provincias inner join (localidades inner join " & _
      ...
      "localidades.idlocalidad=clientes.idlocalidad) on " & _
      "provincias.idprovincia=creditos.idprovincia " & _
      "where " & CondicionFiltro & CondicionCliente & CondicionTipoCreditos & CondicionRangoCreditos & CondicionProvincia & CondicionComercio & CondicionOrden
...
End Sub
Private Function BloquearCredito() As Boolean
'si debe seguir adelante para bloquear, retorna true. Caso contrario (error o desbloqueo), retorna false.
...
If Not VerificarSeleccionLista(lvCreditos) Then Exit Function
...
'Se elimina validación de que sólo una cuota del crédito pueda ser comodín.
...
Call ExportarCreditosTXT
Me.MousePointer = vbDefault
CmdExportarCreditos.Enabled = True
End Sub
Private Sub ExportarCreditosTXT()
Dim Filas As Long
...
Dim cEstadoCredito As String
Dim bIncluirCamposBloqueado As Boolean
Dim cEstadoBloqueo As String
Dim cEstudioBloqueo As String
Dim cFechaEnvio As String
...
If ComboOpciones.Text = "Creditos bloqueados" Then
    bIncluirCamposBloqueado = True
Else
    bIncluirCamposBloqueado = False
End If

Print #1, "Lista de " & ComboOpciones.Text & " a la fecha:" & CStr(CDate(DTPicker5.Value))
If bIncluirCamposBloqueado Then
    Print #1, "Nro.Cliente"; Chr$(9); "DNI"; Chr$(9); "Fecha.Nac."; Chr$(9); "Nro.Prestamo"; Chr$(9); "Fecha alta"; Chr$(9); "Provincia"; Chr$(9); "Comercio"; Chr$(9); "Nombre y Apellido"; Chr$(9); "Capital Otorgado"; Chr$(9); "Tasa"; Chr$(9); "Cantidad Cuotas"; Chr$(9); "Cuota Mensual (*)"; Chr$(9); "Total Prestamo"; Chr$(9); "1º Vto"; Chr$(9); "Saldo Prestamo"; Chr$(9); "Cuotas Cobradas"; Chr$(9); "Cuotas Vencidas"; Chr$(9); "Cuotas Pendientes"; Chr$(9); "Fecha Ultimo Pago"; Chr$(9); "Total cobrado"; Chr$(9); "IVA Exento"; Chr$(9); "Observaciones"; Chr$(9); "Vendedor"; Chr$(9); "Usuario"; Chr$(9); "Estado"; Chr$(9); "Estado Bloqueo"; Chr$(9); "Estudio"; Chr$(9); "Fecha Envío"; Chr$(9); "Observaciones Bloqueo"; Chr$(9); "Situación Bloqueo"
Else
    Print #1, "Nro.Cliente"; Chr$(9); "DNI"; Chr$(9); "Fecha.Nac."; Chr$(9); "Nro.Prestamo"; Chr$(9); "Fecha alta"; Chr$(9); "Provincia"; Chr$(9); "Comercio"; Chr$(9); "Nombre y Apellido"; Chr$(9); "Capital Otorgado"; Chr$(9); "Tasa"; Chr$(9); "Cantidad Cuotas"; Chr$(9); "Cuota Mensual (*)"; Chr$(9); "Total Prestamo"; Chr$(9); "1º Vto"; Chr$(9); "Saldo Prestamo"; Chr$(9); "Cuotas Cobradas"; Chr$(9); "Cuotas Vencidas"; Chr$(9); "Cuotas Pendientes"; Chr$(9); "Fecha Ultimo Pago"; Chr$(9); "Total cobrado"; Chr$(9); "IVA Exento"; Chr$(9); "Observaciones"; Chr$(9); "Vendedor"; Chr$(9); "Usuario"; Chr$(9); "Estado"
End If
   ...
   Do While Not rec.EOF
      ...
      End If
        ...
        If Not IsNull(rec.rdoColumns("FechaFinalizacion")) Then
            cEstadoCredito = ObtenerEstadoCreditoFinalizado(rec.rdoColumns("idcredito"))
        Else
            If Not IsNull(rec.rdoColumns("FechaBloqueo")) Then
                cEstadoCredito = "Bloqueado"
            Else
                cEstadoCredito = "Vigente"
            End If
        End If
       ...
       If bIncluirCamposBloqueado Then
            ...
            sql = "InfoBloqueado " & rec.rdoColumns("idcredito")
            Set rec1 = cnSQL.OpenResultset(sql)
            
            If Not rec1.EOF Then
                Select Case rec1.rdoColumns("Estado")
                Case "EE"
                    cEstadoBloqueo = "Enviar a Estudio"
                Case "NE"
                    cEstadoBloqueo = "No Enviar"
                Case "PE"
                    cEstadoBloqueo = "Pendiente de Envío"
                End Select
                cObservaciones = Trim$(rec1.rdoColumns("Observaciones"))
                If Not IsNull(Trim$(rec1.rdoColumns("Situacion"))) Then
                    cSituacion = Trim$(rec1.rdoColumns("Situacion"))
                End If
                If Not IsNull(rec1.rdoColumns("FechaEnvio")) Then
                    cFechaEnvio = Format(rec1.rdoColumns("FechaEnvio"), "dd/mm/yyyy")
                End If
                If Not IsNull(rec1.rdoColumns("IdEstudio")) Then
                    sql = "SeleccionarEstudio " & CLng(rec1.rdoColumns("IdEstudio"))
                    Set rec2 = cnSQL.OpenResultset(sql)
                    If Not rec2.EOF Then
                        cEstudioBloqueo = rec2.rdoColumns("nombre")
                    End If
                    rec2.Close
                End If
            End If
            ...
            Print #1, rec.rdoColumns("numlegajo") & vbNullString; Chr$(9); rec.rdoColumns("numdocumento") & vbNullString; Chr$(9); rec.rdoColumns("nacimiento") & vbNullString; Chr$(9); rec.rdoColumns("codprestamo"); Chr$(9); CDate(rec.rdoColumns("fechacredito")) & vbNullString; Chr$(9); rec.rdoColumns("provincia") & vbNullString; Chr$(9); rec.rdoColumns("motivobloqueo"); Chr$(9); rec.rdoColumns("cliente") & vbNullString; Chr$(9); Format(rec.rdoColumns("importeafinanciar"), "$0.00") & vbNullString; Chr$(9); rec.rdoColumns("tasa"); Chr$(9); rec.rdoColumns("numcuotas"); Chr$(9); Format(rec.rdoColumns("importecuota"), "$0.00"); Chr$(9); Format(ObtenerTotalCredito(rec.rdoColumns("idcredito")), "$0.00"); Chr$(9); CDate(rec.rdoColumns("fechavencimiento1")); Chr$(9); SaldoCredito; Chr$(9); CuotasCobradas & vbNullString; Chr$(9); CuotasVencidas & vbNullString _
            ; Chr$(9); CuotasPendientes; Chr$(9); Str19; Chr$(9); ImporteParcial; Chr$(9); Str21 & vbNullString; Chr$(9); rec.rdoColumns("observaciones") + "."; Chr$(9); rec.rdoColumns("cad2"); Chr$(9); rec.rdoColumns("cad1"); Chr$(9); cEstadoCredito & Chr$(9); cEstadoBloqueo; Chr$(9); cEstudioBloqueo; Chr$(9); cFechaEnvio; Chr$(9); cObservaciones; vbNullString; Chr$(9); cSituacion; vbNullString
            
       Else
       
            Print #1, rec.rdoColumns("numlegajo") & vbNullString; Chr$(9); rec.rdoColumns("numdocumento") & vbNullString; Chr$(9); rec.rdoColumns("nacimiento") & vbNullString; Chr$(9); rec.rdoColumns("codprestamo"); Chr$(9); CDate(rec.rdoColumns("fechacredito")) & vbNullString; Chr$(9); rec.rdoColumns("provincia") & vbNullString; Chr$(9); rec.rdoColumns("motivobloqueo"); Chr$(9); rec.rdoColumns("cliente") & vbNullString; Chr$(9); Format(rec.rdoColumns("importeafinanciar"), "$0.00") & vbNullString; Chr$(9); rec.rdoColumns("tasa"); Chr$(9); rec.rdoColumns("numcuotas"); Chr$(9); Format(rec.rdoColumns("importecuota"), "$0.00"); Chr$(9); Format(ObtenerTotalCredito(rec.rdoColumns("idcredito")), "$0.00"); Chr$(9); CDate(rec.rdoColumns("fechavencimiento1")); Chr$(9); SaldoCredito; Chr$(9); CuotasCobradas & vbNullString; Chr$(9); CuotasVencidas & vbNullString _
            ; Chr$(9); CuotasPendientes; Chr$(9); Str19; Chr$(9); ImporteParcial; Chr$(9); Str21 & vbNullString; Chr$(9); rec.rdoColumns("observaciones") + "."; Chr$(9); rec.rdoColumns("cad2"); Chr$(9); rec.rdoColumns("cad1"); Chr$(9); cEstadoCredito & vbNullString
...
Me.MousePointer = vbHourglass
'Call ExportarCuotas
Call ExportarCuotasTXT
Me.MousePointer = vbDefault
CmdExportarCuotas.Enabled = True
End Sub
Private Sub ExportarCuotasTXT()
Dim Archivo As String
Dim Archi As String
Dim rec As rdoResultset
Dim sql As String
...
Dim Mes As String
Dim Ano As String
Dim Str20, Str21, Str23, Str24, Str25 As String
Dim RecargoCuota As Currency
Dim ImporteMora As Currency
Dim IvaMora As Currency
...
'titulo principal
Print #1, "Listado de cuotas " & Cadena & " a la fecha:" & CStr(CDate(DTPicker5.Value))
Print #1, "Prestamo"; Chr$(9); "Cuota"; Chr$(9); "Cliente"; Chr$(9); "DNI"; Chr$(9); "Fecha.Nac"; Chr$(9); "Provincia"; Chr$(9); "Comercio"; Chr$(9); "Vencimiento"; Chr$(9); "Capital"; Chr$(9); "Interes"; Chr$(9); "Recargo.Refin."; Chr$(9); "Rec.Vto2"; Chr$(9); "Otorgamiento"; Chr$(9); "Gastos"; Chr$(9); "Seguros"; Chr$(9); "IVA Interes"; Chr$(9); "IVA Seguros"; Chr$(9); "IVA OtorGastos"; Chr$(9); "Total IVA"; Chr$(9); "Saldo"; Chr$(9); "Total Cobrado"; Chr$(9); "Total cuota"; Chr$(9); "IVA Exento"; Chr$(9); "Fecha Alta Credito"; Chr$(9); "Nro.Cupón"; Chr$(9); "Estado"
...
If Not rec.EOF Then
   Do While Not rec.EOF
      ...
      ImporteParcial = ObtenerImporteParcialX(rec.rdoColumns("idcredito"), rec.rdoColumns("numcuota"))
      Str21 = Format(CCur(ImporteParcial), "$0.00")
      ...
      If Not IsNull(rec.rdoColumns("fechacredito")) Then
          Str24 = CDate(rec.rdoColumns("fechacredito"))
      End If
        ...
        If IsNull(rec.rdoColumns("fechacobro")) Then
            ...
            If rec.rdoColumns("vtocambiado") Then
               Str25 = "CambioVto"
            End If
            If rec.rdoColumns("fechavencimiento1") < CDate(DTPicker5) Then
                Str25 = "Vencida"
            End If
        End If
      ...
      'el campo original es logic1
      If rec.rdoColumns("exceptuada") Then
         Str25 = "Exceptuar Mora"
      End If
                  
      Print #1, rec.rdoColumns("codprestamo") & vbNullString; Chr$(9); CStr(rec.rdoColumns("numcuota")) & " de " & CStr(rec.rdoColumns("numcuotas")) & vbNullString; Chr$(9); rec.rdoColumns("cliente") & vbNullString; Chr$(9); rec.rdoColumns("numdocumento") & vbNullString; Chr$(9); rec.rdoColumns("nacimiento") & vbNullString; Chr$(9); rec.rdoColumns("provincia") & vbNullString; Chr$(9); rec.rdoColumns("motivobloqueo"); Chr$(9); CDate(rec.rdoColumns("fechavencimiento1")) & vbNullString; Chr$(9); _
      Format(rec.rdoColumns("importeamortizacion"), "$0.00") & vbNullString; Chr$(9); Format(rec.rdoColumns("importeinteres"), "$0.00") & vbNullString; Chr$(9); Format(rec.rdoColumns("importerefinanciacion"), "$0.00") & vbNullString; Chr$(9); Format(rec.rdoColumns("importerecargovencimiento2"), "$0.00") & vbNullString; Chr$(9); Format(rec.rdoColumns("otorgamiento"), "$0.00") & vbNullString; Chr$(9); Format(rec.rdoColumns("importegastos"), "$0.00") & vbNullString; Chr$(9); Format(rec.rdoColumns("importeseguros"), "$0.00") & vbNullString; Chr$(9); Format(rec.rdoColumns("ivainteres"), "$0.00") & vbNullString; Chr$(9); Format(rec.rdoColumns("ivaseguros"), "$0.00") & vbNullString; Chr$(9); Format(rec.rdoColumns("ivaotorgamientogastos"), "$0.00") & vbNullString; Chr$(9); Format(TotalIva, "$0.00") & vbNullString; Chr$(9); Str20 & vbNullString; Chr$(9); Str21 & vbNullString; Chr$(9); Format(TotalCuota, "$0.00") & vbNullString; Chr$(9); _
      Str23 & vbNullString; Chr$(9); Str24 & vbNullString; Chr$(9); Format(rec.rdoColumns("numfactura"), "0000000"); Chr$(9); Str25 & vbNullString
      Call RefreshTimer
...
Else
   ...
   ComboComercios.ListIndex = -1
   ComboComercios.BackColor = &HFFFFC0
   CheckComercio.Enabled = False
   CheckComercio.Value = False
End If
End Sub
...
TxtTotal.Text = 0
If ComboProvincias.ListIndex >= 0 Then
    IdProvincia = CLng(ComboProvincias.ItemData(ComboProvincias.ListIndex))

    Call CargarComboComercios(ComboComercios, IdProvincia)
    If ComboComercios.ListCount > 0 Then
        CheckComercio.Enabled = True
    Else
        CheckComercio.Enabled = False
        CheckComercio.Value = False
    End If
...
'titulo principal
Print #1, "Listado de saldos de cuotas a la fecha:" & CStr(CDate(DTPicker5.Value))
Print #1, "Prestamo"; Chr$(9); "Cuota"; Chr$(9); "Cliente"; Chr$(9); "Nro.Cliente"; Chr$(9); "DNI"; Chr$(9); "Provincia"; Chr$(9); "Comercio"; Chr$(9); "Vencimiento1"; Chr$(9); "Vencimiento2"; Chr$(9); "Recargo.Refin."; Chr$(9); "Saldo.Capital"; Chr$(9); "Saldo.Interes"; Chr$(9); "Saldo.Gastos"; Chr$(9); "Saldo.Otorg."; Chr$(9); "Saldo.Seg."; Chr$(9); "Saldo.Iva.Int."; Chr$(9); "Saldo.Iva.Seg."; Chr$(9); "Saldo.Iva.Ot."; Chr$(9); "Saldo.Rec.2ºvto"; Chr$(9); "Saldo.Refin."; Chr$(9); "Dias.Mora"; Chr$(9); "Saldo"; Chr$(9); "Estado"
...
If Not rec.EOF Then
   Do While Not rec.EOF
      ...
      Str22 = Format(SaldoCuota, "$0.00")
                   
      Str10 = Format(rec.rdoColumns("importerefinanciacion"), "0.00") & vbNullString
      ...
      Str11 = Format(CapitalRestante, "0.00")
      ...
      Str14 = Format(OtorgamientoRestante, "0.00")
      ...
      Str15 = Format(SeguroRestante, "0.00")
      ...
      Str16 = Format(IvaInteresRestante, "0.00")
      ...
      Str17 = Format(IvaSeguroRestante, "0.00")
      ...
      Str18 = Format(IvaOtorGastoRestante, "0.00")
      ...
      Str19 = Format(Vencimiento2Restante, "0.00")
      ...
      Str20 = Format(RefinRestante, "0.00")
   
      Str21 = Format(DiasMora, "00000")
      ...
      'el campo original es logic1
      If rec.rdoColumns("exceptuada") Then
         Str23 = "Exceptuar Mora"
      End If
      
      'winik
      Print #1, rec.rdoColumns("codprestamo") & vbNullString; Chr$(9); CStr(rec.rdoColumns("numcuota")) & " de " & CStr(rec.rdoColumns("numcuotas")) & vbNullString; Chr$(9); rec.rdoColumns("cliente") & vbNullString; Chr$(9); rec.rdoColumns("numlegajo") & vbNullString; Chr$(9); rec.rdoColumns("numdocumento") & vbNullString; Chr$(9); rec.rdoColumns("provincia") & vbNullString; Chr$(9); rec.rdoColumns("motivobloqueo") & vbNullString; Chr$(9); CDate(rec.rdoColumns("fechavencimiento1")) & vbNullString; Chr$(9); _
      CDate(rec.rdoColumns("fechavencimiento2")) & vbNullString; Chr$(9); Str10 & vbNullString; Chr$(9); Str11 & vbNullString; Chr$(9); Str12 & vbNullString; Chr$(9); Str13 & vbNullString; Chr$(9); Str14 & vbNullString; Chr$(9); Str15 & vbNullString; Chr$(9); Str16 & vbNullString; Chr$(9); Str17 & vbNullString; Chr$(9); Str18 & vbNullString; Chr$(9); Str19 & vbNullString; Chr$(9); Str20 & vbNullString; Chr$(9); Str21 & vbNullString; Chr$(9); Str22 & vbNullString; Chr$(9); _
      Str23 & vbNullString
      Call RefreshTimer
...
'marco
sql = "update cuotas set logic2=1,fechavencimiento1='" & ConvertirFechaSql(CDate(Vencimiento1), "DD/MM/YYYY") & _
...
PopupMenu MDIPrincipal.mnuHistorial

Exit Sub

End Sub
Public Sub ImprimirHistorialCredito(ByVal IdCredito As Long)
...
End Sub
Public Sub ImprimirHistorialPagosCliente(ByVal IdCredito As Long)
'imprime un resumen del credito
'usada en registrar credito
'usada en consultar creditos
Dim sql As String
Dim rec As rdoResultset
Dim IdCliente As Long
Dim Mreporte As New ARHistorialPagosCliente
On Error GoTo merror

sql = "select idcliente from creditos where idcredito=" & CLng(IdCredito)
Set rec = cnSQL.OpenResultset(sql)

If Not rec.EOF Then
    IdCliente = rec.rdoColumns("idcliente")
    rec.Close
    sql = "select ingresos.FechaCobro, ingresos.Secuencia, ingresos.MontoCobradoSecuencia, ingresos.IdCredito, ingresos.NumCuota, ingresos.ImporteCobrado, " & _
          "ingresos.Origen, ingresos.FechaImputacion, ingresos.IdCobrador, ingresos.Usuario, " & _
          "cuotas.numfactura, cuotas.fechavencimiento1, cuotas.fechavencimiento2, cuotas.importevencimiento1, cuotas.importevencimiento2, cobradores.nombre + ' ' + cobradores.apellido as cobrador, " & _
          "creditos.codprestamo, creditos.idcliente " & _
          "from ingresos left join COBRADORES on ingresos.IdCobrador=Cobradores.IdCobrador inner join CREDITOS on CREDITOS.IDCredito=INGRESOS.IdCredito inner join CUOTAS on CUOTAS.IDCredito=INGRESOS.IdCredito and CUOTAS.NumCuota=INGRESOS.NumCuota " & _
          "where creditos.idcliente=" & IdCliente & " order by ingresos.FechaCobro, ingresos.Secuencia"
    
    Set rec = cnSQL.OpenResultset(sql)
    
    If Not rec.EOF Then
       Mreporte.RDODataControl1.Resultset = rec
       Mreporte.Caption = "Historial de pagos del cliente"
       Mreporte.LabelEmpresa = VG_EMPRESA & vbNullString
       Mreporte.LabelTitulo = "Historial de pagos del cliente:"
       Mreporte.LabelCli = ObtenerClienteCredito(IdCredito)
       Mreporte.LabelLe = ObtenerLegajoCliente(IdCredito)
       Mreporte.Show vbModal
    Else
       MsgE "El cliente del crédito no tiene historial de pagos"
    End If
Else
   MsgE "No se encontró el cliente del crédito !!!"
End If

Exit Sub
merror:
tratarerrores "Error imprimiendo el historial de pagos del cliente"
End Sub
